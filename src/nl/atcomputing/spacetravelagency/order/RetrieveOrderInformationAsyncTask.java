...
class RetrieveOrderInformationAsyncTask extends AsyncTask<Order, Order, ArrayList<Order>> {
	
	private Context context;
	private OrderListener listener;
	
	public RetrieveOrderInformationAsyncTask(Context context, OrderListener listener) {
	...
	protected ArrayList<Order> doInBackground(Order... params) {
		...
		for( Order order : params ) {
			...
			HttpPost request = new HttpPost( 
					"http://www.atcomputing.nl/Courselab/spacetravelagency/departureinfo");
	...
	protected void onProgressUpdate(Order... values) {
		this.listener.onDepartureInformationReceived(values[0]);
	}
		...
		try {
			...
			if( ( tokens.length > 1 ) && ( ( tokens.length % 2) == 0) ) {
				for( int i = 0; i < tokens.length; i = i + 2) {
					...
					if ( keyword.contentEquals("departuretime") ) {
